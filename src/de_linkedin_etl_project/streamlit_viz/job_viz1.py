...
def viz_job_details(df_job_details):
    job_category = sorted(
        set(df_job_details['JOB_ROLE'].dropna().unique())
    )
    select_job_category = st.selectbox(
        "Filter by Role", ['All'] + job_category, key="filter_by_role_jobdetails"
    )

    def filter_by_role(df):
        if select_job_category != 'All':
            return df[df['JOB_ROLE'] == select_job_category]
        return df

    filtered_job_table = filter_by_role(df_job_details)
    # 新建展示用列（名字不能重复）
    if not filtered_job_table.empty:
        ...
        st.info('No data for the selected role.')
